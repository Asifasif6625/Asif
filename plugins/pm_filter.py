...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("🔱 🅑︎🅐︎🅒︎🅚︎", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"💌🅟︎🅐︎🅖︎🅔︎💌 \n{round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
    elif off_set is None:
        btn.append([InlineKeyboardButton(f"💌🅟︎🅐︎🅖︎🅔︎💌 \n{round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"), InlineKeyboardButton("🅝︎🅔︎🅧︎🅣︎ 🔱", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def advantage_spoll_choker(bot, query):
    ...
    if k==False:
        ...
        if files:
            ...
            k = await query.message.edit('<b><i>movie not available 🙁<\i><\b>\n<i>reason's 🐿️<\i>\n\n<b>▪️ movie not released \n▪️this movie not added database \n▪️print not available <\b>')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🅾︎ ʙᴜᴛᴛᴏɴꜱ 🅾︎', callback_data='button')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🔑ᴏᴡɴᴇʀ ᴜꜱᴇᴅ ᴄᴏᴍᴍᴀɴᴛ🔑', callback_data='admin')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"💌🅟︎🅐︎🅖︎🅔︎💌 \n1/{round(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="🅝︎🅔︎🅧︎🅣︎ 🔱",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="💌🅜︎🅞︎🅡︎🅔︎ 🅟︎🅐︎🅖︎🅔︎ \n🅝︎🅞︎🅣︎ 🅐︎🅥︎🅐︎🅘︎🅛︎🅘︎🅑︎🅛︎🅔︎💌",callback_data="pages")]
        )
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply("ᴄʜᴇᴄᴋ yᴏᴜʀ ᴍᴏᴠɪᴇ ᴀɴᴅ ᴄʟɪᴄᴋ ʙᴜᴛᴛᴏɴ 👍", reply_markup=InlineKeyboardMarkup(btn))
