        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"⪧⪦\n📍{get_size(file.file_size)}🍭{file.file_name}", callback_data=f'files#{file.file_id}'
                ),
            ]
            for file in files
        ]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.MV_TXT.format(temp.B_NAME),
            reply_markup=reply_markup,
            parse_mode='html'
        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            hmm = await message.reply_photo('https://telegra.ph/file/b7a04bed336187880275e.jpg', cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(9100)            
        except Exception as e:
            logger.exception(e)
            fek = await message.reply_photo('https://telegra.ph/file/b7a04bed336187880275e.jpg', cap, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(9100)            
    else:
        k = await message.reply_photo('https://telegra.ph/file/b7a04bed336187880275e.jpg', cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(9100)
        await k.delete()
    if spoll:
